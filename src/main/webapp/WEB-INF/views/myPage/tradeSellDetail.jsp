...
function endTrade(state){
	var tradeNum='${tradeSellDTO.tradeNum}'
	var mode='sellEnd'
	var url = "<%=cp%>/adminInspectionUpdate";
	$.post(url,{state:state,tradeNum:tradeNum,mode:mode},function(args){
					...
					<div style="color: #222; margin: 0px;  padding: 5px 0px;width: 1000px;">
						<div style="color: #222; margin: 0px; display: flex;">
							<div style="float: left; width: 500px;" align="left" id="title">
								<h3 style="font-size: 28px;letter-spacing: -.36px;margin: 0px;font-weight: 700;">판매내역 > 진행 중</h3>
							</div>
				...
				<div>
					<div>
						<div style="display: flex;-webkit-box-align: center;align-items: center;">
    						...
    						font-weight: 700;"> TS-${tradeSellDTO.tradeNum } </em>
    						</h4>
						</div>
					</div>
					<div style="position: relative;border-top: 2px solid #222;">
						<div style="display: flex;padding: 16px 105px 16px 0;border-bottom: 1px solid #ebebeb;">
							<div style="-webkit-box-flex: 0;flex: none;width: 100px;height: 100px;">
								<div style="overflow: hidden;position: relative;padding-top: 100%;border-radius: 12px;background-color: rgb(244, 244, 244);">
									<img src="/img/${tradeSellDTO.imageName}" 
    						...
    						align-self: center;">
								<strong style="-webkit-box-direction: normal;display: inline-flex;font-size: 14px;">${tradeSellDTO.condition } · ${tradeSellDTO.styleNum }</strong>
								<p style="margin: 0;padding: 0;overflow: hidden;text-overflow: ellipsis;display: -webkit-box;-webkit-line-clamp: 2;
							    -webkit-box-orient: vertical;margin-top: 4px;font-size: 14px;">${tradeSellDTO.engProduct }</p>
							    <span style="margin-top: 4px;font-size: 14px;display: inline-flex;letter-spacing: -.21px;font-weight: 700;
    							color: rgba(34,34,34,.5);">${tradeSellDTO.userSize }</span>
							</div>
						</div>
								...
								<li style="list-style: none;padding: 16px 0 20px;display: table-cell;text-align: center;">
									<div style="position: relative;">
										...
										<p style="text-align: center;margin: 0;">
											<span style="display: inline-block;font-size: 14px;">${tradeSellDTO.immediateBuyPrice }</span>
										</p>
									</div>
								</li>
								<li style="list-style: none;padding: 16px 0 20px;display: table-cell;text-align: center;">
									<div style="position: relative;" class="item_inner">
										...
										<p style="text-align: center;margin: 0;">
											<span style="display: inline-block;font-size: 14px;">${tradeSellDTO.immediateSellPrice }</span>
										</p>
									</div>
								</li>
							</ul>
						...
						<div style="position: absolute;top: 49px;right: 0;">
						    ...
						    border: 1px solid #d3d3d3;color: rgba(34,34,34,.8);"onclick="location.href='<%=cp%>/products?prodNum=${tradeSellDTO.productNum }'">상품 상세보기</div>
						</div>
					</div>
				</div>
				<div>
					<div style="margin-top: 32px;">
						<div style="display: flex;-webkit-box-align: center;align-items: center;">
							<h4 style="padding: 6px 0;margin:0px;font-size: 16px;letter-spacing: -.16px;line-height: 20px;">판매 거래 내역</h4>
						</div>
					</div>
					<div style="border-top: 2px solid #222;">
						<div style="border-bottom: 1px solid #ebebeb;">
							<div style="background-color: #fafafa;"align="left">
								<dl style="margin: 0;padding: 0;font-weight: 700;">
								    ...
								    width: 160px;font-size: 14px;line-height: 28px;">판매 가격</dt>
									<dd style="font-weight: 700;margin: 0;padding: 26px 0;overflow: hidden;font-size: 0;">
    									...
    									vertical-align: top;">${tradeSellDTO.tradeWonPrice }</span>
									</dd>
								</dl>
							</div>
							...
							<div style="margin: 0;padding: 20px 0;border-top: 1px solid #ebebeb;" align="left">
							    ...
							    letter-spacing: -.07px;-webkit-box-pack: start;justify-content: flex-start;">
    								...
    								<c:if test="${tradeSellDTO.tradeSellState=='검수보류'||tradeSellDTO.tradeSellState=='보류' }">
								    	<dd style="margin: 0;padding: 0;white-space: nowrap;color: #ff7500;font-size: 14px;line-height: 20px;letter-spacing: -.2px;">
								    	${tradeSellDTO.tradeSellState }</dd>
							    	</c:if>
    								<c:if test="${tradeSellDTO.tradeSellState=='거래실패' }">
								    	<dd style="margin: 0;padding: 0;white-space: nowrap;color: #f15746;font-size: 14px;line-height: 20px;letter-spacing: -.2px;">
								    	${tradeSellDTO.tradeSellState }</dd>
							    	</c:if>
    								<c:if test="${tradeSellDTO.tradeSellState!='검수보류'&&tradeSellDTO.tradeSellState!='거래실패'&&tradeSellDTO.tradeSellState!='보류' }">
								    	<dd style="margin: 0;padding: 0;white-space: nowrap;color: #222;font-size: 14px;line-height: 20px;letter-spacing: -.2px;">
								    	${tradeSellDTO.tradeSellState }</dd>
							    	</c:if>
							    </dl>
						...
						<c:if test="${tradeSellDTO.tradeSellState=='검수합격' }">
						<div style="display: flex;-webkit-box-pack: center;justify-content: center;">
						    ...
						    background-color: #41b979;border-color: #41b979;color: #fff;margin-left: 8px;"onclick="endTrade('정산완료');">
						    정산 완료 하기</div>
						</div>	
						</c:if>			
						<c:if test="${tradeSellDTO.tradeSellState=='거래실패' }">
						<div style="display: flex;-webkit-box-pack: center;justify-content: center;">
							<div style="display: inline-block;cursor: pointer;vertical-align: middle;text-align: center;padding: 0 18px;height: 42px;
						    line-height: 40px;border-radius: 12px;font-size: 14px;letter-spacing: -.14px;border: 1px solid #222;margin-top: 24px;
						    background-color: #41b979;border-color: #41b979;color: #fff;margin-left: 8px;" onclick="endTrade('취소완료');">
						    거래 취소 하기</div>
						</div>	
						</c:if>	
					</div>
