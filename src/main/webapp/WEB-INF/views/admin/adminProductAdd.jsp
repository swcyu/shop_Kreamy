...
.upload_button:hover,.cancel_button:hover{
	background-color: #222;
	color: #fff;
}
...
$(function(){
	...
	$('#engProduct').on('input', save_check);
	$('#korProduct').on('input', save_check);
	$('#retailPrice').on('input', save_check);
	$('#brandName').on('input', save_check);
	$('#datepicker').on('input', save_check);
	
})
function categoryList(){
	
	var check = document.getElementById("category_list").className
	if(check == 'list_open'){
		document.getElementById("category_list").className="list_close"
	}else{
		document.getElementById("category_list").scrollTop=0;
		document.getElementById("category_list").className="list_open"
	}
}
...
function save_check(){
	var engProduct = document.getElementById('engProduct').value.trim()
	var korProduct = document.getElementById('korProduct').value.trim()
	var retailPrice = document.getElementById('retailPrice').value.trim()
	...
	if(engProduct!='' && korProduct!='' && retailPrice!='' && releaseDate!=''&& brandName!='' && category!='' && check1=='Y'){
		$('#save').attr('class','save_on')	
	}else{
		$('#save').attr('class','save_off')
	}
}
...
function FocusIn(id){
		document.getElementById(id).className="focusIn";
		document.getElementById(id).placeholder="";
}

function FocusOut(id){
	document.getElementById(id).className="focusOut";
	document.getElementById(id).placeholder="";
	if(id=='engProduct'){document.getElementById(id).placeholder="ex) Nike x Stussy WT T-Shirt Black (Asia)";}
	if(id=='korProduct'){document.getElementById(id).placeholder="ex) 나이키 x 스투시 월드투어 티셔츠 블랙 (아시아)";}
	if(id=='styleNum'){document.getElementById(id).placeholder="ex) DD3343-010";}
	if(id=='retailPrice'){document.getElementById(id).placeholder="ex) 123,400원";}
	if(id=='brandName'){document.getElementById(id).placeholder="ex) Nike";}
}

function imageUpload(num) {
    // 업로드 버튼이 클릭되면 파일 찾기 창을 띄운다.
    jQuery("#imageFile"+num).click();
}



function imageTransmit(num) {
	delete_image(num)
    var form = jQuery("ajaxFrom")[0];
    var formData = new FormData(form);
    formData.append("file", jQuery("#imageFile"+num)[0].files[0]);
    formData.append("mode",num);
	var url = "<%=cp%>/adminProductAddPhoto"
    ...
    jQuery.ajax({
        ...
        , success:function(args) {
        	$("#list"+num).html(args);
        	document.getElementById('check'+num).value='Y'
       		if(num=='1'){
       			save_check()
       		}
        }
...
function delete_image(num){
	...
	var check=document.getElementById('check'+num).value
	if(check=='Y'){
		var fileName = document.getElementById('photo'+num).value
		
		var url = "<%=cp%>/adminProductDeletePhoto";
		$.post(url,{fileName:fileName},function(args){
			$('#list'+num).html("<img src='/img/product_default.png' width='150px;height:150px;'/>")	
		});
		document.getElementById('check'+num).value = 'N'
		if(num=='1'){
			save_check()
		}
	}
...
function saveData(){
	...
	if(document.getElementById('save').className=='save_on'){
		...
		var engProduct = document.getElementById("engProduct").value
		
		var url = "<%=cp%>/adminProductEngCheck";
		$.post(url,{engProduct:engProduct},function(args){
			$("#engCheck").html(args);
			if(document.getElementById("eng_prod_check").value=='yes'){
				document.myForm.submit()
			}else{
				alert("이미 목록에 있는 상품 명입니다.")
			}
		});
	}
}
function saveCancel(){
	...
	if(check1=='Y'){
		...
		var url = "<%=cp%>/deleteTest";
		$.post(url,{fileName:fileName},function(args){	
		});
	}
	if(check2=='Y'){
		var fileName = document.getElementById('photo2').value
		var url = "<%=cp%>/deleteTest";
		$.post(url,{fileName:fileName},function(args){	
		});
	}
	if(check3=='Y'){
		var fileName = document.getElementById('photo3').value
		var url = "<%=cp%>/deleteTest";
		$.post(url,{fileName:fileName},function(args){	
		});
	}
	
	$(location).attr('href','<%=cp%>/adminProduct');
		...
		<tr valign="top">
			<td style="width: 160px;">
				...
				<div>
					...
					<ul>
						<li class="menu" onclick="location.href='<%=cp%>/adminProduct'"style="color: black; font-weight: bolder;">상품 관리</li>
						...
						<li class="menu" onclick="location.href='<%=cp%>/adminFAQList'">자주묻는질문 관리</li>
					</ul>
				</div>
			</td>
			...
			<div style="padding-top:5px">
				<div style="color: #222; margin: 0px; display: flex; padding: 5px 0px 16px; border-bottom: 3px solid #222;">
					<div style="float: left; width: 800px;" align="left">
						<h3 style="font-size: 28px;letter-spacing: -.36px;margin: 0px;font-weight: 700;">상품 관리 > 상품 추가</h3>
					</div>
				</div>
				<div align="left">
				<form action="<%=cp %>/adminProductSave" name="myForm" method="post">
					<div style="width: 480px; padding:10px 0px 14px; margin-top: 8px; position: relative;">
						<h3 class="title_black" id="engProduct_title">상품명(영문)</h3>
						<div>
							<input type="text" placeholder="ex) Nike x Stussy WT T-Shirt Black (Asia)" id="engProduct" class="focusOut"
    						name="engProduct" onfocus="FocusIn('engProduct');" onblur="FocusOut('engProduct');"/>
						</div>				
					</div>
					
					<div style="width: 480px; padding:10px 0px 14px; margin-top: 8px; position: relative;">
						<h3 class="title_black" id="korProduct_title">상품명(한글)</h3>
						<div>
							<input type="text" placeholder="ex) 나이키 x 스투시 월드투어 티셔츠 블랙 (아시아)" id="korProduct" class="focusOut"
    						name="korProduct" onfocus="FocusIn('korProduct');" onblur="FocusOut('korProduct');"/>
						</div>				
					</div>
					
					<div style="width: 480px; padding:10px 0px 14px; margin-top: 8px; position: relative;">
						<h3 class="title_black" id="styleNum_title">모델번호</h3>
						<div>
							<input type="text" placeholder="ex) DD3343-010" id="styleNum" class="focusOut"
    						name="styleNum" onfocus="FocusIn('styleNum');" onblur="FocusOut('styleNum');"/>
						</div>				
					</div>
					
					<div style="width: 480px; padding:10px 0px 14px; margin-top: 8px; position: relative;">
						<h3 class="title_black" id="styleNum_title">발매일</h3>
						<div style="position: relative;display: inline-block;cursor: pointer;"onclick="openCalendar();">
							<input type="text" placeholder="선택하세요" id="datepicker" readonly="readonly"class="focusOut" style="cursor: pointer;"
							name="releaseDate"/>
					...
					<div style="width: 480px; padding:10px 0px 14px; margin-top: 8px; position: relative;">
						<h3 class="title_black" id="styleNum_title">발매가</h3>
						<div>
							<input type="text" placeholder="ex) 123,400원" id="retailPrice" class="focusOut" 
    						name="retailPrice" onfocus="FocusIn('retailPrice');" onblur="FocusOut('retailPrice');"/>
						</div>			
					</div>
					
					<div style="width: 480px; padding:10px 0px 14px; margin-top: 8px; position: relative;">
						<h3 class="title_black" id="styleNum_title">브랜드 명</h3>
						<div>
							<input type="text" placeholder="ex) Nike" id="brandName" class="focusOut"
    						name="brandName" onfocus="FocusIn('brandName');" onblur="FocusOut('brandName');"/>
						</div>			
					</div>
					
					<div style="width: 480px; padding:10px 0px 14px; margin-top: 8px; position: relative;">
						<h3 class="title_black" id="colorway_title">대표색상</h3>
						<div>
							<input type="text" placeholder="ex) WHITE/BLACK" id="colorway" class="focusOut"
    						name="colorway" onfocus="FocusIn('colorway');" onblur="FocusOut('colorway');"/>
						</div>			
					</div>
					
					<div style="width: 480px; padding:10px 0px 14px; margin-top: 8px; position: relative;">
						<h3 style="color: #222; box-sizing: border-box;margin: 0; padding: 0;font-size: 13px;
    					letter-spacing: -.07px; line-height: 18px;font-weight: 700;">카테고리</h3>
						<div style="position: relative; cursor: pointer;" onclick="categoryList();">
							<input type="text" placeholder="선택하세요" id="category" class="focusOut" readonly="readonly" style="cursor: pointer;"
							name="category"/>
						...
						<div class="list_close" id="category_list" onblur="categoryList()">
							<div class="category" style="padding: 11px 50px 10px 16px; font-size: 13px; color: #222;"align="left" onclick="category('스니커즈');">스니커즈</div>
							<div class="category" style="padding: 11px 50px 10px 16px; font-size: 13px; color: #222;"align="left" onclick="category('의류');">의류</div>
							<div class="category" style="padding: 11px 50px 10px 16px; font-size: 13px; color: #222;"align="left" onclick="category('패션잡화');">패션잡화</div>
							<div class="category" style="padding: 11px 50px 10px 16px; font-size: 13px; color: #222;"align="left" onclick="category('라이프');">라이프</div>
							<div class="category" style="padding: 11px 50px 10px 16px; font-size: 13px; color: #222;"align="left" onclick="category('테크');">테크</div>
						</div>		
					</div>
					
					
					<div style="width: 550px; padding:10px 0px 14px; margin-top: 8px; position: relative;">
						<h3 style="color: #222; box-sizing: border-box;margin-bottom: 10px; padding: 0;font-size: 13px;
    					letter-spacing: -.07px; line-height: 18px;font-weight: 700;">이미지 선택</h3>
						<div style="display: inline-flex ;">
						
							<div style="display: inline-block;width: 150px;">
								<div style="width: 150px;height: 150px;" id="list1"><img src='/img/product_default.png' width="150px;height:150px;"/></div>
								<div class="upload_button" style="width: 150px; border: 1px solid #222; border-radius: 12px;margin-top: 5px;cursor: pointer;"
								align="center"onclick="imageUpload('1');">메인사진(필수)</div>
								<div class="cancel_button" style="width: 150px; border: 1px solid #222; border-radius: 12px;margin-top: 5px;cursor: pointer;"
								align="center"onclick="delete_image('1');">삭제</div>
							</div>
							
							<div style="display: inline-block;width: 150px;margin-left: 50px;">
								<div style="width: 150px;height: 150px;" id="list2"><img src='/img/product_default.png' width="150px;height:150px;"/></div>
								<div class="upload_button" style="width: 150px; border: 1px solid #222; border-radius: 12px;margin-top: 5px;cursor: pointer;"
								align="center"onclick="imageUpload('2');">세부사진1</div>
								<div class="cancel_button" style="width: 150px; border: 1px solid #222; border-radius: 12px;margin-top: 5px;cursor: pointer;"
								align="center"onclick="delete_image('2');">삭제</div>
							</div>
							
							<div style="display: inline-block;width: 150px;margin-left: 50px;">
								<div style="width: 150px;height: 150px;" id="list3"><img src='/img/product_default.png' width="150px;height:150px;"/></div>
								<div class="upload_button" style="width: 150px; border: 1px solid #222; border-radius: 12px;margin-top: 5px;cursor: pointer;"
								align="center"onclick="imageUpload('3');">세부사진2</div>
								<div class="cancel_button" style="width: 150px; border: 1px solid #222; border-radius: 12px;margin-top: 5px;cursor: pointer;"
								align="center"onclick="delete_image('3');">삭제</div>
							</div>
							
						</div>
					</div>
					
					<div align="center" style="display:flex; width: 550px; ">
						<div style="width: 120px; padding-top:20px;margin-right :2px;margin-left:150px;display: inline-block;" align="center">
							<div class="save_cancel"  onclick="saveCancel();">취소</div>
						</div>
			
						<div style="width: 120px; padding-top:20px;margin-left :8px;display: inline-block;" align="center" id="saveButton">
...
<input type="file" id="imageFile1" onChange="imageTransmit('1');" style="display:none;"/>
<input type="file" id="imageFile2" onChange="imageTransmit('2');" style="display:none;"/>
<input type="file" id="imageFile3" onChange="imageTransmit('3');" style="display:none;"/>

<input type="hidden" id="check1" value="N">
<input type="hidden" id="check2" value="N">
<input type="hidden" id="check3" value="N">

<input type="hidden" id="engCheck" value="no">
